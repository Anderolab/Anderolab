...
if isempty(GroupVariable)
    Mean = nanmean(TableInput.(Variable), 1);
    SDs = nanstd(TableInput.(Variable), 1);
...
elseif length(string(GroupVariable)) == 1
    ...
    for g_ix = unique(TableInput.(GroupVariable)).'
        ...
        Means(c, :) = ... % Group mean for all animals
            nanmean(TableInput.(Variable)(TableInput.(GroupVariable) ...
            == g_ix, :), 1);
        SDs(c, :) = ... % Group sd for all animals
            nanstd(TableInput.(Variable)(TableInput.(GroupVariable) ...
            == g_ix, :), [], 1);
    ...
    for gr_1 = unique(TableInput.(GroupVariable(1))).'
        ...
        for gr_2 = Labels
            ...
            Means(c, :) = nanmean(TableInput.(Variable)(ix, :), 1);
            SDs(c, :) = nanstd(TableInput.(Variable)(ix, :), [], 1);
            Ns(c) = sum(ix == 1);
